...
def get_available_texture_patterns(folder_path="textures"):
    """
    Lists all available texture patterns stored as image files in the specified folder and its subfolders.
    
    Args:
        folder_path (str): Path to the folder containing subfolders with texture image files.

    Returns:
        list: A list of available texture pattern filenames with their full paths, including valid extensions.
    """
    ...
    # List to store texture patterns
    available_textures = []
    ...
    for root, _, files in os.walk(folder_path):
        for file in files:
            ...
            if ext in valid_extensions:
                # Store the full path of the texture file (including extension)
                full_path = os.path.join(root, file)
                available_textures.append(full_path)
...
def mask_face(image, face_location, six_points, angle, args, type="surgical"):
    ...
    if args["mask_y_offset"]:
        # Adjust six_points for vertical offset
        six_points[:, 1] += args["mask_y_offset"]
        
    # Create a rotation matrix
    center = (image.shape[1] // 2, image.shape[0] // 2)  # Center of the image
    rotation_matrix = cv2.getRotationMatrix2D(center, args["mask_rotation_angle"], 1.0)
    ...
    ################################# ADDED CODE ################################# #################################
    # Rotate the mask
    img = cv2.warpAffine(img, rotation_matrix, (img.shape[1], img.shape[0])) #rotating the image
    ...
    if args['pattern']:
        if args['pattern'] != "random":
            ...
            available_texture = get_available_texture_patterns("masks/textures")
            # print(available_texture)
            texture = random.choice(available_texture)
            # print(texture)
            img = texture_the_mask(img, texture,  args['pattern_weight'])
    ...
    if args['color']:
        # Apply color to mask
        # print("Color selected: ", args['color'])
    ...
    #dst_mask = cv2.warpPerspective(img, M, (h, w)) # COMMENTED THIS LINE
    
    ################################# ADDED CODE ##################################################################
    dst_mask = cv2.warpPerspective(img, M, (image.shape[1], image.shape[0]))
     ################################################################## #################################
...
def mask_image(image_in, args):
   
    # image = cv2.imread(image_path)
    original_image = image_in.copy()
    ...
    if len(face_locations) == 0:
        ...
        print("#"*15)
        print("dlib unable to detect face in image to put mask on")
        print("#"*15)
        ...
        for (i, face_location) in enumerate(face_locations):
           ...
           # print("Face landmarks are: ", face_landmarks)
            # draw_landmarks(face_landmarks, image)
            six_points_on_face, angle = get_six_points(face_landmarks, image_in)
            mask = []
            if mask_type != "all":
                if len(masked_images) > 0:
                    image_in = masked_images.pop(0)
                image_in, mask_binary = mask_face(
                    image_in, face_location, six_points_on_face, angle, args, type=mask_type
                )
                # compress to face tight
                face_height = face_location[2] - face_location[0]
                face_width = face_location[1] - face_location[3]
                masked_images.append(image_in)
